...
public class ComponentGoldenIsland extends AetherStructure {
            ...
            for (int l2 = 24; l2 >= -24; l2--) {
                for (int i3 = -24; i3 <= 24; i3++) {
                    ...
                    if (Math.sqrt(k3 * k3 + i4 * i4 + k4 * k4) <= 24.0D) {
                        if (BlocksAether.isGood(
                                this.getBlockStateWithOffset(i2, l2 + 1, i3)
                            )
                            && l2 > 4) {
                            this.setBlockWithOffset(
                                i2, l2, i3, BlocksAether.aether_grass, 3
                            );
                            this.setBlockWithOffset(
                                i2, l2 - 1, i3, BlocksAether.aether_dirt, 0
                            );
                            this.setBlockWithOffset(
                                i2,
                            ...
                            if (l2 >= 24 / 2) {
                                ...
                                if (j5 < 2) {
                                    //AetherGenUtils.generateGoldenOakTree(this, i2, l2 +
                                    //1, i3);
                                } else if (j5 == 3) {
                                    if (this.random.nextInt(2) == 0) {
                                        //new
                                        //WorldGenLakes(Blocks.FLOWING_WATER).generate(world,
                                        //random, new BlockPos.MutableBlockPos((i2 + i +
                                        //random.nextInt(3)) - random.nextInt(3), l2 + j,
                                        //(i3 + k + random.nextInt(3)) -
                                        //random.nextInt(3)));
                                    }
                                } else if (j5 == 4) {
                                    if (this.random.nextInt(2) == 0) {
                                        AetherGenUtils.generateFlower(
                                            ...
                                            Blocks.red_flower,
                                            0,
                                            (i2 + this.random.nextInt(3))
                                                - this.random.nextInt(3),
                                            l2 + 1,
                                            (i3 + this.random.nextInt(3))
                                                - this.random.nextInt(3)
                                        );
                                    } else {
                                        AetherGenUtils.generateFlower(
                        ...
                        } else if (BlocksAether.isGood(
                                       this.getBlockStateWithOffset(i2, l2, i3)
                                   )) {
                            this.setBlockWithOffset(
                                i2, l2, i3, BlocksAether.holystone, 0
                            );
                        }
                    }
                }
    ...
    public void generateCaves(int x, int y, int z, int size) {
        ...
        for (int l = 0; l <= size; l++) {
            ...
            double d10
                = (double) (MathHelper.sin(((float) l * 3.141593F) / (float) size) + 1.0F)
                    * d9
                + 1.0D;
            double d11
                = (double) (MathHelper.sin(((float) l * 3.141593F) / (float) size) + 1.0F)
