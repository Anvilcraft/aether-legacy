...
public class AetherGenUtils {
    public static void generateFlower(
        ...
        for (int i = 0; i < 64; ++i) {
            ...
            if (structure.getBlockStateWithOffset(x, y, z).getMaterial() == Material.air
                && y < 255
                && structure.getBlockStateWithOffset(x, y - 1, z)
    ...
    public static void generateGoldenOakTree(
        AetherStructure structure, int offsetX, int offsetY, int offsetZ
    ) {
        int x = offsetX;
        int y = offsetY;
        int z = offsetZ;

        if (structure.getBlockStateWithOffset(x, y - 1, z) != BlocksAether.aether_grass
            && structure.getBlockStateWithOffset(x, y - 1, z)
            ...
            for (int y1 = y + 5; y1 < y + 12; y1++) {
                for (int z1 = z - 3; z1 < z + 4; z1++) {
                    if ((x1 - x) * (x1 - x) + (y1 - y - 8) * (y1 - y - 8)
                        ...
                        structure.setBlockWithOffset(
                            x1, y1, z1, BlocksAether.golden_oak_leaves, 0
                        );
                    }
                }
            }
    ...
    private static void
    goldenOakBranch(AetherStructure structure, int x, int y, int z, int slant) {
    ...
    public static void generateClouds(
        ...
        for (int n = 0; n < cloudSize; ++n) {
            ...
            if (structure.random.nextBoolean() && !isFlat
                || isFlat && structure.random.nextInt(10) == 0) {
                y += structure.random.nextInt(3) - 1;
            }

            z += structure.random.nextInt(3) - 1 + zTendency;

            for (int x1 = x; x1 < x + structure.random.nextInt(4) + 3 * (isFlat ? 3 : 1);
                 ++x1) {
                for (int y1 = y; y1 < y + structure.random.nextInt(1) + 2; ++y1) {
                    for (int z1 = z;
                         z1 < z + structure.random.nextInt(4) + 3 * (isFlat ? 3 : 1);
                         ++z1) {
                        if (structure.getBlockState(x1, y1, z1).getMaterial()
                            == Material.air) {
