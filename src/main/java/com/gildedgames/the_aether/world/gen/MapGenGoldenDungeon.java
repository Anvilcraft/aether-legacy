...
public class MapGenGoldenDungeon extends MapGenStructure {
    ...
    public static class Start extends StructureStart {
        ...
        private void create(World worldIn, Random random, int chunkX, int chunkZ) {
            ...
            ComponentGoldenIsland dungeon
                = new ComponentGoldenIsland((chunkX << 4) + 2, (chunkZ << 4) + 2);
            ...
            for (int stubIslands = 0; stubIslands < this.stubIslandCount; ++stubIslands) {
                ...
                int k5 = -MathHelper.floor_double(
                    24.0D * (double) random.nextFloat() * 0.29999999999999999D
                );
                int i6 = MathHelper.floor_double(-Math.sin(f1 * f2) * (double) f3);

                this.components.add(new ComponentGoldenIslandStub(
                    (chunkX << 4) + 2, (chunkZ << 4) + 2, l4, k5, i6, 8
                ));
            }

            this.components.add(new ComponentGoldenDungeon(
                (chunkX << 4) + 2, (chunkZ << 4) + 2, this.dungeonDirection
            ));
