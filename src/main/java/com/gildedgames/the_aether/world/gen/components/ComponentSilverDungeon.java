...
public class ComponentSilverDungeon extends AetherStructure {
    ...
    private static final Block LOCKED_LIGHT_ANGELIC_STONE
        = BlocksAether.locked_light_angelic_stone;
    ...
    public ComponentSilverDungeon(int chunkX, int chunkZ) {
        ...
        this.boundingBox = new StructureBoundingBox(
            chunkX, 80, chunkZ, chunkX + 100, 220, chunkZ + 100
        );
    }
    ...
    public boolean generate() {
        ...
        for (y = 0; y < 2; y++) {
            for (z = 7 + 7 * this.finalStaircaseZ; z < 9 + 7 * this.finalStaircaseZ;
                 z++) {
                        ...
                        if (type >= 3) {
                            ...
                            switch (roomType) {
                                case 1: {
                                    ...
                                    if (this.getBlockState(u, 5 * q + 1, v).getMaterial()
                                        == Material.air) {
                                        this.setBlockWithOffset(
                                            u, 5 * q + 1, v, Blocks.chest, 0
                                        );

                                        TileEntity tileEntity
                                            = this.getTileEntityFromPosWithOffset(
                                                u, 5 * q + 1, v
                                            );
        ...
        for (x = 0; x < 24; x++) {
            for (z = 0; z < 20; z++) {
                int distance = (int
                ) (Math.sqrt(x * x + (z - 7) * (z - 7))
                   + Math.sqrt(x * x + (z - 12) * (z - 12)));

                if (distance == 21) {
                    this.setBlockWithOffset(
                        26 + x, 0, 5 + z, LOCKED_LIGHT_ANGELIC_STONE, 0
                    );
                } else if (distance > 21) {
        ...
        for (x = 0; x < 2; x++) {
            for (z = 0; z < 2; z++) {
                this.setBlockWithOffset(
                    44 + x * 5, 2, 11 + z * 7, BlocksAether.ambrosium_torch, 0
                );
            }
        }
        ...
        EntityValkyrieQueen valkyrieQueen = new EntityValkyrieQueen(
            this.worldObj,
            (double) this.getActualX(40, 15),
            (double) this.getActualY(1) + 0.5D,
            (double) this.getActualZ(40, 15)
        );

        valkyrieQueen.setPosition(
            this.getActualX(40, 15), this.getActualY(2), this.getActualZ(40, 15)
        );
        valkyrieQueen.setDungeon(
            this.getActualX(26, 24), this.getActualY(0), this.getActualZ(26, 24)
        );
    ...
    public void generateGoldenOakSapling(int x, int y, int z) {
        ...
        for (int lineX = x; lineX < x + 3; lineX += 2) {
            for (int lineZ = z; lineZ < z + 3; lineZ += 2) {
                this.setBlockWithOffset(
                    lineX, y + 1, lineZ, BlocksAether.ambrosium_torch, 0
                );
            }
        }
    }
    ...
    public void generateColumn(int x, int y, int z, int yRange) {
        ...
        this.setBlockWithOffset(
            x + 1, y + (yRange - 1), z + 1, BlocksAether.pillar_top, 0
        );
    }
        ...
        switch (item) {
            case 0:
                return new ItemStack(
                    ItemsAether.gummy_swet, random.nextInt(15) + 1, random.nextInt(2)
                );
            case 1:
