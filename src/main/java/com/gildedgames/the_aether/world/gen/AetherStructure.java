...
public abstract class AetherStructure extends StructureComponent {
    ...
    public void setBlocks(
        Block blockState, int meta, Block extraBlockState, int extraMeta, int chance
    ) {
        this.blockState = blockState;
        this.meta = meta;
    ...
    public void addLineX(int x, int y, int z, int xRange) {
        for (int lineX = x; lineX < x + xRange; lineX++) {
            Block block = this.getBlockState(
                lineX + this.startX, y + this.startY, z + this.startZ
            );
    ...
    public void addLineY(int x, int y, int z, int yRange) {
        for (int lineY = y; lineY < y + yRange; lineY++) {
            Block block = this.getBlockState(
                x + this.startX, lineY + this.startY, z + this.startZ
            );
    ...
    public void addLineZ(int x, int y, int z, int zRange) {
        for (int lineZ = z; lineZ < z + zRange; lineZ++) {
            Block block = this.getBlockState(
                x + this.startX, y + this.startY, lineZ + this.startZ
            );
    ...
    public void addPlaneX(int x, int y, int z, int yRange, int zRange) {
        for (int lineY = y; lineY < y + yRange; lineY++) {
            for (int lineZ = z; lineZ < z + zRange; lineZ++) {
                Block block = this.getBlockState(
                    x + this.startX, lineY + this.startY, lineZ + this.startZ
                );
    ...
    public void addPlaneY(int x, int y, int z, int xRange, int zRange) {
        for (int lineX = x; lineX < x + xRange; lineX++) {
            for (int lineZ = z; lineZ < z + zRange; lineZ++) {
                Block block = this.getBlockState(
                    lineX + this.startX, y + this.startY, lineZ + this.startZ
                );
    ...
    public void addPlaneZ(int x, int y, int z, int xRange, int yRange) {
        for (int lineX = x; lineX < x + xRange; lineX++) {
            for (int lineY = y; lineY < y + yRange; lineY++) {
                Block block = this.getBlockState(
                    lineX + this.startX, lineY + this.startY, z + this.startZ
                );
    ...
    public void
    addSquareTube(int x, int y, int z, int xRange, int yRange, int zRange, int angel) {
        Block temp1 = this.blockState;
        Block temp2 = this.extraBlockState;
    ...
    public void addSolidBox(int x, int y, int z, int xRange, int yRange, int zRange) {
        for (int lineX = x; lineX < x + xRange; lineX++) {
            for (int lineY = y; lineY < y + yRange; lineY++) {
                for (int lineZ = z; lineZ < z + zRange; lineZ++) {
                    Block block = this.getBlockState(
                        lineX + this.startX, lineY + this.startY, lineZ + this.startZ
                    );
    ...
    public TileEntity getTileEntityFromPosWithOffset(int x, int y, int z) {
        return !this.dungeonBoundingBox.isVecInside(
                   this.getActualX(x, z), this.getActualY(y), this.getActualZ(x, z)
               )
            ? null
    ...
    public Block getBlockState(int x, int y, int z) {
        return this.getBlockAtCurrentPosition(
            this.worldObj, x, y, z, this.dungeonBoundingBox
        );
    }
    ...
    public void setBlock(int x, int y, int z, Block state, int meta) {
        this.placeBlockAtCurrentPosition(
            this.worldObj, state, meta, x, y, z, this.dungeonBoundingBox
        );
    }
    ...
    public boolean
    spawnEntity(Entity entity, int structureX, int structureY, int structureZ) {
