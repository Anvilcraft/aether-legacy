...
public class AetherBiomeDecorator extends BiomeDecorator {
    ...
    public AetherGenDungeonOakTree golden_oak_tree_dungeon
        = new AetherGenDungeonOakTree();
    ...
    @Override
    public void
    decorateChunk(World worldIn, Random random, BiomeGenBase biome, int x, int z) {
        if (this.world != null) {
    ...
    protected void genDecorations(BiomeGenBase biome) {
        ...
        if ((AetherConfig.shouldLoadHolidayContent())
            || (AetherConfig.allowSeasonalChristmas()
        ...
        if (AetherConfig.tallgrassEnabled()) {
            for (int i3 = 0; i3 < 10; ++i3) {
                ...
                if (k14 > 0) {
                    ...
                    this.aetherBiome.getRandomWorldGenForGrass(this.rand).generate(
                        this.world, this.rand, j7, l17, i11
                    );
                }
            }
        ...
        if (this.shouldSpawn(10)) {
            (new WorldGenLakes(Blocks.water))
                .generate(
                    this.world,
                    this.rand,
                    this.chunk_X + this.rand.nextInt(16) + 8,
                    this.rand.nextInt(256),
                    this.chunk_Z + this.rand.nextInt(16) + 8
                );
        }
    }
    ...
    public WorldGenerator getTree() {
        return this.shouldSpawn(30) ? new AetherGenOakTree()
                                    : new AetherGenSkyrootTree(true);
    }
    ...
    public void generateFoilage(Block block) {
        ...
        for (int n = 0; n < 2; n++) {
            this.foilage.generate(
                this.world,
                this.rand,
                this.chunk_X + this.nextInt(16) + 8,
                this.nextInt(128),
                this.chunk_Z + this.nextInt(16) + 8
            );
        }
    }
    ...
    public void generateClouds(int meta, int amount, int chance, int y) {
        if (this.shouldSpawn(chance)) {
            ...
            this.clouds.generate(
                this.world,
                this.rand,
                this.chunk_X + this.nextInt(16),
                this.nextInt(y),
                this.chunk_Z + this.nextInt(16)
            );
        }
    }
