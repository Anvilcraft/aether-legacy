...
public class ChunkProviderAether implements IChunkProvider {
    ...
    private MapGenLargeColdAercloud largeColdAercloudStructure
        = new MapGenLargeColdAercloud();
        ...
        for (int i1 = 0; i1 < 2; i1++) {
            for (int j1 = 0; j1 < 2; j1++) {
                for (int k1 = 0; k1 < 32; k1++) {
                    ...
                    double d6
                        = (this.buffer[(i1 * 3 + (j1 + 1)) * 33 + (k1 + 1)] - d2) * 0.25D;
                    double d7
                        = (this.buffer[((i1 + 1) * 3 + j1) * 33 + (k1 + 1)] - d3) * 0.25D;
                    double d8
                        = (this.buffer[((i1 + 1) * 3 + (j1 + 1)) * 33 + (k1 + 1)] - d4)
                        * 0.25D;
    ...
    private double[] setupNoiseGenerators(double buffer[], int x, int z) {
        ...
        this.pnr = this.perlinNoise1.generateNoiseOctaves(
            this.pnr, x, 0, z, 3, 33, 3, d / 80D, d1 / 160D, d / 80D
        );
        this.ar
            = this.noiseGen1.generateNoiseOctaves(this.ar, x, 0, z, 3, 33, 3, d, d1, d);
        this.br
            = this.noiseGen1.generateNoiseOctaves(this.br, x, 0, z, 3, 33, 3, d, d1, d);
    ...
    public void recreateStructures(int x, int z) {
        this.largeColdAercloudStructure.func_151539_a(
            this, this.worldObj, x, z, (Block[]) null
        );
        this.silverDungeonStructure.func_151539_a(
            this, this.worldObj, x, z, (Block[]) null
        );
        this.goldenDungeonStructure.func_151539_a(
            this, this.worldObj, x, z, (Block[]) null
        );
    }
    ...
    public ChunkPosition func_147416_a(
        World worldIn, String structureName, int x, int y, int z
    ...
    public void populate(IChunkProvider provider, int chunkX, int chunkZ) {
        ...
        this.largeColdAercloudStructure.generateStructuresInChunk(
            this.worldObj, this.rand, chunkX, chunkZ
        );
        this.silverDungeonStructure.generateStructuresInChunk(
            this.worldObj, this.rand, chunkX, chunkZ
        );
        this.goldenDungeonStructure.generateStructuresInChunk(
            this.worldObj, this.rand, chunkX, chunkZ
        );

        biome.decorate(this.worldObj, this.rand, x, z);

        this.dungeon_bronze.generate(
            this.worldObj, this.rand, x, this.rand.nextInt(48) + 24, z
        );

        SpawnerAnimals.performWorldGenSpawning(
            this.worldObj, biome, x + 8, z + 8, 16, 16, this.rand
        );
    }
