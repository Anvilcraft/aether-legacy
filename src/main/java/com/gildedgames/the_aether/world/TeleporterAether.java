...
public class TeleporterAether extends Teleporter {
    ...
    public void
    placeInPortal(Entity entityIn, double x, double y, double z, float rotationYaw) {
        if (!this.portalSpawn) {
    ...
    public boolean placeInExistingPortal(
        Entity entityIn, double x, double y, double z, float rotationYaw
    ) {
        short short1 = 128;
        double d3 = -1.0D;
        int i = 0;
        ...
        if (this.destinationCoordinateCache.containsItem(j1)) {
            AetherPortalPosition portalposition = (AetherPortalPosition
            ) this.destinationCoordinateCache.getValueByKey(j1);
            d3 = 0.0D;
            ...
            for (l3 = l - short1; l3 <= l + short1; ++l3) {
                ...
                for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1) {
                    ...
                    for (int i2 = this.worldServerInstance.getActualHeight() - 1; i2 >= 0;
                         --i2) {
                        if (this.worldServerInstance.getBlock(l3, i2, l1)
        ...
        if (d3 >= 0.0D) {
            if (flag) {
                this.destinationCoordinateCache.add(
                    j1,
                    new AetherPortalPosition(
                        i, j, k, this.worldServerInstance.getTotalWorldTime()
                    )
                );
            ...
            if (this.worldServerInstance.getBlock(i - 1, j, k)
                == BlocksAether.aether_portal) {
                i4 = 2;
            }
            ...
            if (this.worldServerInstance.getBlock(i, j, k - 1)
                == BlocksAether.aether_portal) {
                i4 = 3;
            }
            ...
            if (i4 > -1) {
                ...
                boolean flag1
                    = !this.worldServerInstance.isAirBlock(i + l2 + j3, j, k + i3 + k3)
                ...
                if (flag1 && flag2) {
                    ...
                    int k1 = k - k3;
                    d7 -= (double) k3;
                    flag1 = !this.worldServerInstance.isAirBlock(
                                l3 + l2 + j3, j, k1 + i3 + k3
                            )
                        || !this.worldServerInstance.isAirBlock(
                            l3 + l2 + j3, j + 1, k1 + i3 + k3
                        );
                ...
                entityIn.rotationYaw
                    = rotationYaw - (float) (j2 * 90) + (float) (i4 * 90);
            } else {
                entityIn.motionX = entityIn.motionY = entityIn.motionZ = 0.0D;
            }

            entityIn.setLocationAndAngles(
                d11, d6, d7, entityIn.rotationYaw, entityIn.rotationPitch
            );
            return true;
        } else {
            return false;
            ...
            label274:
                ...
                for (i3 = this.worldServerInstance.getActualHeight() - 1; i3 >= 0; --i3) {
                    if (this.worldServerInstance.isAirBlock(i2, i3, k2)) {
                        while (i3 > 0
                               && this.worldServerInstance.isAirBlock(i2, i3 - 1, k2)) {
                            --i3;
                        }
        ...
        if (d0 < 0.0D) {
            for (i2 = i - b0; i2 <= i + b0; ++i2) {
                ...
                label222:
                    ...
                    for (i3 = this.worldServerInstance.getActualHeight() - 1; i3 >= 0;
                         --i3) {
                        if (this.worldServerInstance.isAirBlock(i2, i3, k2)) {
                            while (i3 > 0
                                   && this.worldServerInstance.isAirBlock(i2, i3 - 1, k2)
                ...
                for (j3 = 1; j3 < 3; ++j3) {
                    for (k3 = -1; k3 < 3; ++k3) {
                        ...
                        this.worldServerInstance.setBlock(
                            l3, i4, j4, flag ? Blocks.glowstone : Blocks.air
                        );
                    }
                }
            }
            ...
            for (j3 = 0; j3 < 4; ++j3) {
                for (k3 = -1; k3 < 4; ++k3) {
                    ...
                    this.worldServerInstance.setBlock(
                        ...
                        i4,
                        j4,
                        (Block) (flag ? Blocks.glowstone : BlocksAether.aether_portal),
                        0,
                        2
                    );
                }
    ...
    public void removeStalePortalLocations(long worldTime) {
        if (worldTime % 100L == 0L) {
            ...
            while (iterator.hasNext()) {
                ...
                Teleporter.PortalPosition portalposition = (Teleporter.PortalPosition
