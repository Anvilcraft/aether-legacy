...
public class PlayerAetherEvents {
    ...
    public void onPlayerAetherConstructing(EntityConstructing event) {
        if (event.entity instanceof EntityPlayer) {
            event.entity.registerExtendedProperties(
                "aether_legacy:player_aether", new PlayerAether()
            );
        } else if (event.entity instanceof EntityLivingBase) {
            event.entity.registerExtendedProperties(
                "aether_legacy:entity_hook", new EntityHook()
            );
        }
    }
    ...
    public void onPlayerAetherLoggedIn(PlayerLoggedInEvent event) {
        if (!event.player.worldObj.isRemote) {
            ...
            AetherNetwork.sendTo(
                new PacketAccessory(playerAether), (EntityPlayerMP) event.player
            );
    ...
    public void onPlayerAetherClone(Clone event) {
        ...
        if (!event.wasDeath
            || event.entityPlayer.worldObj.getGameRules().getGameRuleBooleanValue(
                "keepInventory"
            )) {
    ...
    public void onPlayerAetherDrops(LivingDropsEvent event) {
        if (event.entityLiving instanceof EntityPlayer
            && !event.entityLiving.worldObj.getGameRules().getGameRuleBooleanValue(
                "keepInventory"
            )) {
    ...
    public void onPlayerAetherUpdate(LivingUpdateEvent event) {
        if (event.entityLiving instanceof EntityPlayer) {
            ...
            ((EntityHook
             ) event.entityLiving.getExtendedProperties("aether_legacy:entity_hook"))
                .onUpdate();
        }
    }
    ...
    public void onLivingAttack(LivingAttackEvent event) {
        if (event.entityLiving instanceof EntityPlayer) {
            ...
            if (playerAether.getAccessoryInventory().isWearingPhoenixSet()
                && event.source.isFireDamage()) {
                event.setCanceled(true);
            } else if (playerAether.getAbilities().get(3).shouldExecute()) {
                event.setCanceled(((AbilityRepulsion) playerAether.getAbilities().get(3))
                                      .onPlayerAttacked(event.source));
            }
        }
    }
    ...
    public void onAchievementGet(AchievementEvent event) {
        ...
        int achievementType = achievement == AchievementsAether.defeat_bronze ? 1
            : achievement == AchievementsAether.defeat_silver                 ? 2
                                                                              : 0;

        if (!player.worldObj.isRemote
            && ((EntityPlayerMP) player).func_147099_x().canUnlockAchievement(achievement)
            ...
            AetherNetwork.sendTo(
                new PacketAchievement(achievementType), (EntityPlayerMP) player
            );
        }
    }
}