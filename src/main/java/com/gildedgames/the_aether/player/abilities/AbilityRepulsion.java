...
public class AbilityRepulsion implements IAetherAbility {
    ...
    public boolean shouldExecute() {
        return this.player.getAccessoryInventory().wearingAccessory(
            new ItemStack(ItemsAether.repulsion_shield)
        );
    }
    ...
    public void onUpdate() {
        ...
        List<?> entities
            = this.player.getEntity().worldObj.getEntitiesWithinAABBExcludingEntity(
        ...
        for (int size = 0; size < entities.size(); ++size) {
            ...
            if (isProjectile(projectile)
                && this.getShooter(projectile) != this.player.getEntity()) {
                double x, y, z;
                ...
                packX = (-projectile.motionX * 0.15F)
                    + ((this.rand.nextFloat() - 0.5F) * 0.05F);
                packY = (-projectile.motionY * 0.15F)
                    + ((this.rand.nextFloat() - 0.5F) * 0.05F);
                packZ = (-projectile.motionZ * 0.15F)
                    + ((this.rand.nextFloat() - 0.5F) * 0.05F);
                ...
                this.player.getEntity().worldObj.playSoundAtEntity(
                    this.player.getEntity(), "note.snare", 1.0F, 1.0F
                );
                this.player.getAccessoryInventory().damageWornStack(
                    1, new ItemStack(ItemsAether.repulsion_shield)
                );
            }
        }
    }
    ...
    private Entity getShooter(Entity ent) {
        return ent instanceof EntityArrow    ? ((EntityArrow) ent).shootingEntity
            ...
            : ent instanceof EntityProjectileBase
            ? ((EntityProjectileBase) ent).getThrower()
