...
public class ItemMoaEgg extends Item {
    ...
    public boolean onItemUse(
        ...
        if (player.capabilities.isCreativeMode) {
            EntityMoa moa = new EntityMoa(
                world,
                AetherAPI.instance().getMoaType(stack.getTagCompound().getInteger("typeId"
                ))
            );
    ...
    public void getSubItems(Item item, CreativeTabs tab, List subItems) {
        for (int moaTypeSize = 0; moaTypeSize < AetherAPI.instance().getMoaTypeSize();
             ++moaTypeSize) {
            ItemStack stack = new ItemStack(this);
            NBTTagCompound compound = new NBTTagCompound();
            AetherMoaType moaType = AetherAPI.instance().getMoaType(moaTypeSize);

            if (moaType.getCreativeTab() == tab || tab == null
    ...
    public int getColorFromItemStack(ItemStack stack, int renderPass) {
        ...
        if (tag != null) {
            AetherMoaType moaType
                = AetherAPI.instance().getMoaType(tag.getInteger("typeId"));
