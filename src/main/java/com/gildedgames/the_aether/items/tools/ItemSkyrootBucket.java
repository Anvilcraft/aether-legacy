...
public class ItemSkyrootBucket extends Item {
    ...
    public void registerIcons(IIconRegister registry) {
        ...
        this.waterBucket
            = registry.registerIcon(Aether.find("misc/buckets/skyroot_water_bucket"));
        this.poisonBucket
            = registry.registerIcon(Aether.find("misc/buckets/skyroot_poison_bucket"));
        this.remedyBucket
            = registry.registerIcon(Aether.find("misc/buckets/skyroot_remedy_bucket"));
        this.milkBucket
            = registry.registerIcon(Aether.find("misc/buckets/skyroot_milk_bucket"));
    }
    ...
    public IIcon getIconFromDamage(int meta) {
        return meta == 1 ? this.waterBucket
            ...
            : meta == 3  ? this.remedyBucket
            : meta == 4  ? this.milkBucket
                         : this.itemIcon;
    }
    ...
    public int getItemStackLimit(ItemStack stack) {
        return EnumSkyrootBucketType.getType(stack.getItemDamage())
    ...
    public String getUnlocalizedName(ItemStack itemstack) {
        ...
        return this.getUnlocalizedName() + "_"
            + EnumSkyrootBucketType.getType(meta).toString();
    }
    ...
    public boolean onItemUseFirst(
        ...
        if (world.getBlock(
            ...
            && !world.isRemote) {
            BlockCauldron cauldron = (BlockCauldron) world.getBlock(
                movingobjectposition.blockX,
                movingobjectposition.blockY,
                movingobjectposition.blockZ
            );

            int waterLevel = BlockCauldron.func_150027_b(world.getBlockMetadata(
                movingobjectposition.blockX,
                movingobjectposition.blockY,
                movingobjectposition.blockZ
            ));
                    ...
                    if (!player.capabilities.isCreativeMode) {
                        player.inventory.setInventorySlotContents(
                            ...
                            new ItemStack(
                                ...
                                1,
                                EnumSkyrootBucketType.Empty.meta
                            )
                        );
                    }
    ...
    @Override
    public ItemStack
    onItemRightClick(ItemStack heldItem, World world, EntityPlayer player) {
        ...
        MovingObjectPosition movingobjectposition
            = this.getMovingObjectPositionFromPlayer(world, player, flag);

        if (movingobjectposition == null) {
            return heldItem;
        } else {
            FillBucketEvent event
                = new FillBucketEvent(player, heldItem, world, movingobjectposition);
            ...
            if (movingobjectposition.typeOfHit
                ...
                if (flag) {
                    if (!player.canPlayerEdit(
                            i, j, k, movingobjectposition.sideHit, heldItem
                        )) {
                        return heldItem;
                    }
                    ...
                    if (material == Material.water && l == 0) {
                        ...
                        return this.fillBucket(
                            heldItem, player, ItemsAether.skyroot_bucket
                        );
                    }
                } else {
    ...
    public boolean tryPlaceContainedLiquid(
        EntityPlayer player, World world, ItemStack stack, int x, int y, int z
    ) {
        if (EnumSkyrootBucketType.getType(stack.getItemDamage())
    ...
    public ItemStack
    onBucketUsed(ItemStack itemstack, World world, EntityPlayer entityplayer) {
        ...
        if (EnumSkyrootBucketType.getType(meta) == EnumSkyrootBucketType.Poison) {
            entityplayer.addPotionEffect(
                new EffectInebriation(PotionInebriation.inebriation.id, 500, 0)
            );
        } else if (EnumSkyrootBucketType.getType(meta) == EnumSkyrootBucketType.Remedy) {
            player.setCured(200);
            if (!world.isRemote) {
                entityplayer.curePotionEffects(new ItemStack(
                    ItemsAether.skyroot_bucket, EnumSkyrootBucketType.Remedy.meta
                ));
            }
