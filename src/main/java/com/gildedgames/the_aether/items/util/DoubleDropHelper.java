...
public class DoubleDropHelper {
    public static void
    dropBlock(EntityPlayer player, int x, int y, int z, Block block, int meta) {
        ...
        if (block.canSilkHarvest(player.worldObj, player, x, y, z, meta)
            && EnchantmentHelper.getEnchantmentLevel(
                   Enchantment.silkTouch.effectId, stack
               ) > 0) {
            ...
            ForgeEventFactory.fireBlockHarvesting(
                items, player.worldObj, block, x, y, z, meta, 0, 1.0f, true, player
            );
        ...
        if (skyrootTool.getDigSpeed(stack, block, meta)
            == skyrootTool.getEffectiveSpeed()) {
    ...
    protected static void dropBlockAsItem(
        World p_149642_1_,
        int p_149642_2_,
        int p_149642_3_,
        int p_149642_4_,
        ...
        if (!p_149642_1_.isRemote
            && p_149642_1_.getGameRules().getGameRuleBooleanValue("doTileDrops")
            && !p_149642_1_.restoringBlockSnapshots) // do not drop items while restoring
                                                     // blockstates, prevents item dupe
        {
            float f = 0.7F;
            double d0 = (double) (p_149642_1_.rand.nextFloat() * f)
                + (double) (1.0F - f) * 0.5D;
            double d1 = (double) (p_149642_1_.rand.nextFloat() * f)
                + (double) (1.0F - f) * 0.5D;
            double d2 = (double) (p_149642_1_.rand.nextFloat() * f)
                + (double) (1.0F - f) * 0.5D;
