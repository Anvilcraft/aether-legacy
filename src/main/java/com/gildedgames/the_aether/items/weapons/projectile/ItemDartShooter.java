...
public class ItemDartShooter extends Item {
    ...
    public void registerIcons(IIconRegister registry) {
        this.goldenIcon
            = registry.registerIcon(Aether.find("projectile/golden_dart_shooter"));
        this.poisonIcon
            = registry.registerIcon(Aether.find("projectile/poison_dart_shooter"));
        this.enchantedIcon
            = registry.registerIcon(Aether.find("projectile/enchanted_dart_shooter"));
    }
    ...
    public String getUnlocalizedName(ItemStack itemstack) {
        return this.getUnlocalizedName() + "_"
            + EnumDartShooterType.getType(itemstack.getItemDamage()).toString();
    }

    @Override
    public ItemStack
    onItemRightClick(ItemStack heldItem, World world, EntityPlayer entityplayer) {
        int consume;

        if (!(entityplayer.capabilities.isCreativeMode)) {
            consume = this.consumeItem(
                entityplayer, ItemsAether.dart, heldItem.getItemDamage()
            );
        } else {
