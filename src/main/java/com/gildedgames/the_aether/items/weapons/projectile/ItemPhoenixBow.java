...
public class ItemPhoenixBow extends ItemBow {
    ...
    public void onPlayerStoppedUsing(
        ItemStack stack, World worldIn, EntityPlayer entityLiving, int timeLeft
    ) {
        if (entityLiving instanceof EntityPlayer) {
            EntityPlayer entityplayer = (EntityPlayer) entityLiving;
            boolean flag = entityplayer.capabilities.isCreativeMode
                || EnchantmentHelper.getEnchantmentLevel(
                       Enchantment.infinity.effectId, stack
                   ) > 0;
            ...
            if (itemstack != null || flag) {
                ...
                if ((double) f >= 0.1D) {
                    ...
                    if (!worldIn.isRemote) {
                        EntityPhoenixArrow entityarrow = this.createArrow(
                            worldIn, f * 2.0F, itemstack, entityplayer
                        );
                        ...
                        if (j > 0) {
                            entityarrow.setDamage(
                                entityarrow.getDamage() + (double) j * 0.5D + 0.5D
                            );
                        }
                        ...
                        int l = EnchantmentHelper.getEnchantmentLevel(
                            Enchantment.flame.effectId, stack
                        );
    ...
    public EntityPhoenixArrow createArrow(
        World worldIn, float distance, ItemStack stack, EntityLivingBase shooter
    ) {
        EntityPhoenixArrow entityPhoenixArrow
            = new EntityPhoenixArrow(worldIn, shooter, distance);
    ...
    @Override
    public ItemStack
    onItemRightClick(ItemStack heldItem, World worldIn, EntityPlayer playerIn) {
