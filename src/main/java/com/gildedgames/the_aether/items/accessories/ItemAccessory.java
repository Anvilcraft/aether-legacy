...
public class ItemAccessory extends Item {
    ...
    public void registerIcons(IIconRegister registry) {
        ...
        for (int i = 0; i < ClientProxy.ACCESSORY_ICONS.length; ++i) {
            ClientProxy.ACCESSORY_ICONS[i] = registry.registerIcon(Aether.find(
                "slots/" + ((AccessoryType) orderedList.func_148745_a(i)).getDisplayName()
            ));
        }
    }
    ...
    public static ItemStack dispenseAccessory(IBlockSource blockSource, ItemStack stack) {
        EnumFacing enumfacing
            = BlockDispenser.func_149937_b(blockSource.getBlockMetadata());
        ...
        List<EntityLivingBase> list = blockSource.getWorld().getEntitiesWithinAABB(
    ...
    @Override
    public ItemStack
    onItemRightClick(ItemStack stack, World worldIn, EntityPlayer player) {
        ItemStack heldItem = player.getHeldItem();

        if (heldItem != null) {
            if (PlayerAether.get(player).getAccessoryInventory().setAccessorySlot(
                    heldItem.copy()
                )) {
                --heldItem.stackSize;
    ...
    public boolean getIsRepairable(ItemStack toRepair, ItemStack repair) {
        return (repair.getItem() == ItemsAether.zanite_gemstone
                && toRepair.getItem() == ItemsAether.zanite_ring)
            || (repair.getItem() == ItemsAether.zanite_gemstone
                && toRepair.getItem() == ItemsAether.zanite_pendant);
    }
    ...
    public ItemAccessory setInactiveTexture(String location) {
        this.texture_inactive = new ResourceLocation(
            "aether_legacy", "textures/armor/accessory_" + location + ".png"
        );
