...
public abstract class AetherTileEntity
    extends TileEntity implements ISidedInventory, IInventory {
    ...
    public void setInventorySlotContents(int index, ItemStack stack) {
        boolean flag = stack != null && this.getStackInSlot(index) != null
            && stack.isItemEqual(this.getStackInSlot(index))
            && ItemStack.areItemStackTagsEqual(stack, this.getStackInSlot(index));
        ...
        for (int i = 0; i < list.tagCount(); ++i) {
            ...
            if (slot >= 0 && slot < this.getTileInventory().size()) {
                this.setInventorySlotContents(
                    slot, ItemStack.loadItemStackFromNBT(stackCompound)
                );
            }
        }
