...
public class TileEntityIncubator extends AetherTileEntity {
    ...
    private final FilledList<ItemStack> incubatorItemStacks
    ...
    public void updateEntity() {
        ...
        if (this.progress >= this.ticksRequired) {
            if (this.getStackInSlot(1).getItem() instanceof ItemMoaEgg) {
                ...
                if (!this.worldObj.isRemote) {
                    ...
                    for (int safeY = 0; !this.worldObj.isAirBlock(
                             this.xCoord, this.yCoord + safeY, this.zCoord
                         );
                         safeY++) {
                        moa.setPositionAndUpdate(
                            this.xCoord + 0.5D,
                            this.yCoord + safeY + 1.5D,
                            this.zCoord + 0.5D
                        );
                    }
                ...
                AetherHooks.onMoaHatched(
                    moaEgg.getMoaTypeFromItemStack(this.getStackInSlot(1)), this
                );
            }
        ...
        if (this.powerRemaining <= 0) {
            if (this.getStackInSlot(0) != null && this.getStackInSlot(1) != null
                && this.getStackInSlot(1).getItem() == ItemsAether.moa_egg
                && this.getStackInSlot(0).getItem()
                    == Item.getItemFromBlock(BlocksAether.ambrosium_torch)) {
        ...
        if (flag != this.isIncubating()) {
            ...
            BlockAetherContainer.setState(
                this.worldObj, this.xCoord, this.yCoord, this.zCoord, this.isIncubating()
            );
        }
    }
