...
public class TileEntityEnchanter extends AetherTileEntity {
    ...
    private final FilledList<ItemStack> enchantedItemStacks
        = new FilledList<ItemStack>(3, null);
    ...
    public void updateEntity() {
        ...
        if (this.currentEnchantment != null) {
            if (this.progress >= this.ticksRequired) {
                if (!this.worldObj.isRemote) {
                    ...
                    EnchantmentHelper.setEnchantments(
                        EnchantmentHelper.getEnchantments(this.getStackInSlot(0)), result
                    );
                    ...
                    if (this.getStackInSlot(2) != null
                        && this.getStackInSlot(2).isStackable()) {
                    ...
                    if (this.getStackInSlot(0).getItem().hasContainerItem(
                        ...
                        this.setInventorySlotContents(
                            0,
                            this.getStackInSlot(0).getItem().getContainerItem(
                                this.getStackInSlot(0)
                            )
                        );
                    } else {
            ...
            if (this.getStackInSlot(0) == null
                || (this.getStackInSlot(0) != null
                    && AetherAPI.instance().getEnchantment(this.getStackInSlot(0))
                        != this.currentEnchantment)) {
            ...
            if (this.powerRemaining <= 0) {
                if (this.getStackInSlot(1) != null
                    && AetherAPI.instance().isEnchantmentFuel(this.getStackInSlot(1))) {
            ...
            AetherEnchantment enchantment
                = AetherAPI.instance().getEnchantment(itemstack);

            if (enchantment != null) {
                if (this.getStackInSlot(2) == null
                    || (enchantment.getOutput().getItem()
                            == this.getStackInSlot(2).getItem()
                        && enchantment.getOutput().getItemDamage()
                            == this.getStackInSlot(2).getItemDamage()
                        && this.getStackInSlot(2).isStackable())) {
                    ...
                    this.ticksRequired = AetherHooks.onSetEnchantmentTime(
                        this, this.currentEnchantment, this.ticksRequired
                    );
                }
            }
        }
        ...
        if (flag != this.isEnchanting()) {
            this.markDirty();
            BlockAetherContainer.setState(
                this.worldObj, this.xCoord, this.yCoord, this.zCoord, this.isEnchanting()
            );
        }
    }
