...
public class ContainerAccessories extends ContainerPlayer {
    ...
    public ContainerAccessories(IAccessoryInventory inventory, EntityPlayer player) {
        ...
        for (int x = 1; x < 3; x++) {
            for (int y = 0; y < 4; y++) {
                AccessoryType type
                    = (AccessoryType) this.orderedList.func_148745_a(slotID);

                this.addSlotToContainer(new SlotAccessory(
                    inventoryInstance, slotID, type, 59 + x * 18, 8 + y * 18, player
                ));
                slotID++;
            }
        }
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int slotNumber) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (!(slot instanceof SlotAccessory) && !(slot instanceof SlotCrafting)) {
                ...
                if (newSlotIndex != -1) {
                    Slot accessorySlot
                        = (SlotAccessory) this.inventorySlots.get(newSlotIndex);
