...
public class InventoryAccessories implements IAccessoryInventory {
    ...
    public float getCurrentPlayerStrVsBlock(float original) {
        ...
        if (this.wearingAccessory(new ItemStack(ItemsAether.zanite_pendant))) {
            f
                *= (1F
                    + ((float) (this.getStackInSlot(AccessoryType.PENDANT).getItemDamage()
                       )
                       / ((float
                          ) (this.getStackInSlot(AccessoryType.PENDANT).getMaxDamage())
                          * 3F)));
        }

        if (this.getStackInSlot(AccessoryType.RING) != null
            && this.getStackInSlot(AccessoryType.RING).getItem()
                == ItemsAether.zanite_ring) {
            f
                *= (1F
                    + ((float) (this.getStackInSlot(AccessoryType.RING).getItemDamage())
                       / ((float) (this.getStackInSlot(AccessoryType.RING).getMaxDamage())
                          * 3F)));
        }

        if (this.getStackInSlot(AccessoryType.EXTRA_RING) != null
            && this.getStackInSlot(AccessoryType.EXTRA_RING).getItem()
                == ItemsAether.zanite_ring) {
            f
                *= (1F
                    + ((float
                       ) (this.getStackInSlot(AccessoryType.EXTRA_RING).getItemDamage())
                       / ((float
                          ) (this.getStackInSlot(AccessoryType.EXTRA_RING).getMaxDamage())
                          * 3F)));
        }
    ...
    public void markDirty() {
        if (!this.playerAether.getEntity().worldObj.isRemote
            && this.playerAether.getEntity() instanceof EntityPlayer) {
    ...
    public boolean isItemValidForSlot(int slot, ItemStack stack) {
        if (stack != null && stack.getItem() instanceof ItemAccessory) {
            return ((this.orderedList.func_148745_a(slot)
                     == ((ItemAccessory) stack.getItem()).getType())
                    || (this.orderedList.func_148745_a(slot)
                        == ((ItemAccessory) stack.getItem()).getExtraType()))
        ...
        if (stack.getItem() instanceof ItemAccessory) {
            if (this.getStackInSlot(type) == null) {
                this.setInventorySlotContents(
                    this.orderedList.func_148747_b(type), stack
                );
            } else if (this.getStackInSlot(((ItemAccessory) stack.getItem()).getExtraType()) == null) {
                this.setAccessorySlot(
                    ((ItemAccessory) stack.getItem()).getExtraType(), stack
                );
            }
        }
    }
    ...
    public boolean wearingAccessory(ItemStack stack) {
        ...
        for (int i = 0; i < this.getSizeInventory(); ++i) {
            ...
            if (!flag && accessoryStack != null
                && accessoryStack.getItem() == stack.getItem()) {
                flag = true;
            }
        }
    ...
    public boolean wearingArmor(ItemStack stack) {
        if (this.playerAether.getEntity() instanceof EntityPlayer) {
            ...
            for (int i = 0; i < player.inventory.armorInventory.length; ++i) {
                ...
                if (!flag && armorStack != null
                    && armorStack.getItem() == stack.getItem()) {
                    flag = true;
                }
            }
