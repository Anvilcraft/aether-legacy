...
public class PacketDisplayDialogue extends AetherPacket<PacketDisplayDialogue> {
    ...
    public PacketDisplayDialogue(
        String dialogueName, String dialogue, String... dialogueText
    ) {
    ...
    public void handleClient(PacketDisplayDialogue message, EntityPlayer player) {
        FMLClientHandler.instance().getClient().displayGuiScreen(new GuiServerDialogue(
            message.dialogueName, message.dialogue, message.dialogueText
        ));
    }
