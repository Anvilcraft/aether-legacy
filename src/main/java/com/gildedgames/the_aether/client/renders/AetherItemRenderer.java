...
public class AetherItemRenderer extends ItemRenderer {
    ...
    public void
    renderFirstPersonArm(RenderPlayer renderPlayer, EntityClientPlayerMP playerIn) {
        ...
        if (gloves.getItem() instanceof ItemAccessory) {
            this.mc.getTextureManager().bindTexture(
                ((ItemAccessory) gloves.getItem()).texture
            );
            ...
            renderPlayer.modelBipedMain.setRotationAngles(
                0.0F, 0.0F, 0.0F, 0.0F, 0.0F, 0.0625F, playerIn
            );
    ...
    public void renderItemInFirstPerson(float partialTicks) {
        float f1 = this.prevEquippedProgress
        ...
        float f4 = entityplayersp.prevRenderArmYaw
            + (entityplayersp.renderArmYaw - entityplayersp.prevRenderArmYaw)
                * partialTicks;
        GL11.glRotatef(
            (entityclientplayermp.rotationPitch - f3) * 0.1F, 1.0F, 0.0F, 0.0F
        );
        ...
        if (itemstack != null && itemstack.getItem() instanceof ItemMap) {
            ...
            tessellator.addVertexWithUV(
                (double) (0 - b0), (double) (128 + b0), 0.0D, 0.0D, 1.0D
            );
            tessellator.addVertexWithUV(
                (double) (128 + b0), (double) (128 + b0), 0.0D, 1.0D, 1.0D
            );
            tessellator.addVertexWithUV(
                (double) (128 + b0), (double) (0 - b0), 0.0D, 1.0D, 0.0D
            );
            tessellator.addVertexWithUV(
                (double) (0 - b0), (double) (0 - b0), 0.0D, 0.0D, 0.0D
            );
            tessellator.draw();

            IItemRenderer custom
                = MinecraftForgeClient.getItemRenderer(itemstack, FIRST_PERSON_MAP);
            MapData mapdata
                = ((ItemMap) itemstack.getItem()).getMapData(itemstack, this.mc.theWorld);

            if (custom == null) {
                if (mapdata != null) {
                    this.mc.entityRenderer.getMapItemRenderer().func_148250_a(
                        mapdata, false
                    );
                }
            } else {
            ...
            if (entityclientplayermp.getItemInUseCount() > 0) {
                ...
                if (enumaction == EnumAction.eat || enumaction == EnumAction.drink) {
                    f6 = (float) entityclientplayermp.getItemInUseCount() - partialTicks
                        + 1.0F;
                ...
                GL11.glTranslatef(
                    -f7 * 0.4F,
                    MathHelper.sin(MathHelper.sqrt_float(f5) * (float) Math.PI * 2.0F)
                        * 0.2F,
                    -f6 * 0.2F
                );
            }
            ...
            if (entityclientplayermp.getItemInUseCount() > 0) {
                ...
                if (enumaction1 == EnumAction.block) {
                    ...
                    f10 = (float) itemstack.getMaxItemUseDuration()
                        - ((float) entityclientplayermp.getItemInUseCount() - partialTicks
                           + 1.0F);
                    f11 = f10 / 20.0F;
                    ...
                    if (f11 > 0.1F) {
                        GL11.glTranslatef(
                            0.0F,
                            MathHelper.sin((f10 - 0.1F) * 1.3F) * 0.01F * (f11 - 0.1F),
                            0.0F
                        );
                    }
            ...
            if (itemstack.getItem().requiresMultipleRenderPasses()) {
                ...
                for (int x = 1;
                     x < itemstack.getItem().getRenderPasses(itemstack.getItemDamage());
                     x++) {
        ...
        if (Loader.isModLoaded("battlegear2")) {
            mods.battlegear2.client.utils.BattlegearRenderHelper.renderItemInFirstPerson(
                partialTicks, mc, this
            );
        }
    }
