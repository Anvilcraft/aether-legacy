...
public class PlayerAetherRenderer {
    private static final ResourceLocation TEXTURE_VALKYRIE
        = Aether.locate("textures/entities/valkyrie/valkyrie.png");

    private static final ResourceLocation TEXTURE_HALO
        = Aether.locate("textures/other/halo.png");
    ...
    public int renderAetherArmor(
        ...
        if (stack != null) {
            ...
            if (item instanceof ItemAetherArmor) {
                ...
                this.mc.getTextureManager().bindTexture(RenderBiped.getArmorResource(
                    playerAether.getEntity(), stack, slotType, null
                ));
                ModelBiped modelbiped = slotType == 2 ? renderPlayer.modelArmor
                                                      : renderPlayer.modelArmorChestplate;
                ...
                modelbiped = net.minecraftforge.client.ForgeHooksClient.getArmorModel(
                    playerAether.getEntity(), stack, slotType, modelbiped
                );
    ...
    public void renderAccessories(
        ...
        this.modelMisc.heldItemRight = this.modelWings.heldItemRight
            = this.modelGlow.heldItemRight = itemstack != null ? 1 : 0;
        ...
        if (itemstack != null && playerAether.getEntity().getItemInUseCount() > 0) {
            ...
            if (enumaction == EnumAction.block) {
                this.modelMisc.heldItemRight = this.modelWings.heldItemRight
                    = this.modelGlow.heldItemRight = 3;
            } else if (enumaction == EnumAction.bow) {
                this.modelMisc.aimedBow = this.modelWings.aimedBow
                    = this.modelGlow.aimedBow = true;
            }
        }

        this.modelMisc.isSneak = this.modelWings.isSneak = this.modelWings.isSneak
            = playerAether.getEntity().isSneaking();

        this.doRender(playerAether, x, y, z, partialTicks);

        this.modelMisc.aimedBow = this.modelWings.aimedBow = this.modelGlow.aimedBow
            = false;
        this.modelMisc.isSneak = this.modelWings.isSneak = this.modelGlow.isSneak = false;
        this.modelMisc.heldItemRight = this.modelWings.heldItemRight
            = this.modelGlow.heldItemRight = 0;
    }

    private void doRender(
        PlayerAether playerAether, double x, double y, double z, float partialTicks
    ) {
        EntityPlayer player = playerAether.getEntity();

        this.modelMisc.onGround = this.modelWings.onGround = this.modelGlow.onGround
            = playerAether.getEntity().getSwingProgress(partialTicks);
        this.modelMisc.isRiding = this.modelWings.isRiding = this.modelGlow.isRiding
            = playerAether.getEntity().isRiding();
        this.modelMisc.isChild = this.modelWings.isChild = this.modelGlow.isChild
            = playerAether.getEntity().isChild();

        GL11.glPushMatrix();
        GL11.glDisable(GL11.GL_CULL_FACE);

        float f2 = this.interpolateRotation(
            player.prevRenderYawOffset, player.renderYawOffset, partialTicks
        );
        float f3 = this.interpolateRotation(
            player.prevRotationYawHead, player.rotationYawHead, partialTicks
        );
        float f4;
        ...
        float f13 = player.prevRotationPitch
        ...
        float f6 = player.prevLimbSwingAmount
    ...
    private void renderModel(
        ...
        this.setCapeRendering(
            playerAether.getAccessoryInventory().getStackInSlot(AccessoryType.CAPE)
            != null
        );

        this.modelMisc.setRotationAngles(
            limbSwing,
            prevLimbSwing,
            rotation,
            interpolateRotation,
        ...
        this.modelWings.setRotationAngles(
            limbSwing,
            prevLimbSwing,
            rotation,
            interpolateRotation,
        ...
        this.modelHalo.setRotationAngles(
            limbSwing,
            prevLimbSwing,
            rotation,
            interpolateRotation,
        ...
        this.modelGlow.setRotationAngles(
            limbSwing,
            prevLimbSwing,
            rotation,
            interpolateRotation,
        ...
        if (playerAether.getAccessoryInventory().getStackInSlot(AccessoryType.PENDANT)
            != null) {
            ItemAccessory pendant = (ItemAccessory) playerAether.getAccessoryInventory()
                                        .getStackInSlot(AccessoryType.PENDANT)
                                        .getItem();
        ...
        if (playerAether.getAccessoryInventory().getStackInSlot(AccessoryType.CAPE)
                != null
            && !playerAether.getAccessoryInventory().wearingAccessory(
                new ItemStack(ItemsAether.invisibility_cape)
            )) {
            ItemAccessory cape = (ItemAccessory) playerAether.getAccessoryInventory()
                                     .getStackInSlot(AccessoryType.CAPE)
                                     .getItem();
            ...
            if (!player.isInvisible()) {
                if (playerAether.shouldRenderCape) {
                    ...
                    int colour = cape.getColorFromItemStack(
                        playerAether.getAccessoryInventory().getStackInSlot(
                            AccessoryType.CAPE
                        ),
                        0
                    );
        ...
        if (playerAether.getAccessoryInventory().getStackInSlot(AccessoryType.GLOVES)
            != null) {
            ItemAccessory gloves = (ItemAccessory) playerAether.getAccessoryInventory()
                                       .getStackInSlot(AccessoryType.GLOVES)
                                       .getItem();

            this.mc.getTextureManager().bindTexture(gloves.texture);

            int colour = gloves.getColorFromItemStack(
                playerAether.getAccessoryInventory().getStackInSlot(AccessoryType.GLOVES),
                0
            );
            float red = ((colour >> 16) & 0xff) / 255F;
            float green = ((colour >> 8) & 0xff) / 255F;
        ...
        if (playerAether.getAccessoryInventory().getStackInSlot(AccessoryType.SHIELD)
            != null) {
            ItemAccessory shield = (ItemAccessory) playerAether.getAccessoryInventory()
                                       .getStackInSlot(AccessoryType.SHIELD)
                                       .getItem();

            if (player.motionX == 0.0
                && (player.motionY == -0.0784000015258789 || player.motionY == 0.0)
                && player.motionZ == 0.0 && shield.hasInactiveTexture()) {
        ...
        if (AetherRankings.isRankedPlayer(player.getUniqueID())
            && PlayerAether.get(player).shouldRenderHalo && !player.isInvisible()) {
        ...
        if (player.getUniqueID().toString().equals("cf51ef47-04a8-439a-aa41-47d871b0b837")
            || AetherRankings.isDeveloper(player.getUniqueID())
                && playerAether.shouldRenderGlow && !player.isInvisible()) {
    ...
    protected void rotateCorpse(
        EntityLivingBase entity, float p_77043_2_, float p_77043_3_, float partialTicks
    ) {
        ...
        if (entity.deathTime > 0) {
            ...
            if ((s.equals("Dinnerbone") || s.equals("Grumm"))
                && (!(entity instanceof EntityPlayer)
                    || !((EntityPlayer) entity).getHideCape())) {
    ...
    private float
    interpolateRotation(float p_77034_1_, float p_77034_2_, float p_77034_3_) {
        float f3;
