...
public class AetherOverlay {
    private static final ResourceLocation TEXTURE_JUMPS
        = Aether.locate("textures/gui/jumps.png");

    private static final ResourceLocation TEXTURE_COOLDOWN_BAR
        = Aether.locate("textures/gui/cooldown_bar.png");

    private static final ResourceLocation TEXTURE_POISON_VIGNETTE
        = Aether.locate("textures/blur/poison_vignette.png");

    private static final ResourceLocation TEXTURE_CURE_VIGNETTE
        = Aether.locate("textures/blur/cure_vignette.png");
    ...
    public static void renderPoison(Minecraft mc) {
        ...
        if (playerAether.poisonTime > 0) {
            ...
            tessellator.addVertexWithUV(
                (double) width, (double) height, -90.0D, 1.0D, 1.0D
            );
    ...
    public static void renderIronBubble(Minecraft mc) {
        ...
        int bubbleAmount = PlayerAether.get(mc.thePlayer)
                               ...
                               .getAccessoryCount(new ItemStack(ItemsAether.iron_bubble));
        ...
        if (mc.playerController.shouldDrawHUD() && mc.thePlayer.isInWater()
            ...
            for (int i = 0; i < bubbleAmount; ++i) {
                drawTexturedModalRect(
                    (width / 2 - 8 * i) + 81, height - 49, 16, 18, 9, 9
                );
            }
        }
    ...
    public static void renderCooldown(Minecraft mc) {
        ...
        if (playerInfo.getHammerCooldown() != 0) {
            ...
            int cooldownRemaining = (int
            ) ((float) (playerInfo.getHammerCooldown())
               / (float) (playerInfo.getHammerMaxCooldown()) * 128F);
            int width = scaledresolution.getScaledWidth();

            mc.fontRenderer.drawStringWithShadow(
                playerInfo.getHammerName() + " "
                    + I18n.format("item.notch_hammer.cooldown"),
                (width / 2)
                    - (mc.fontRenderer.getStringWidth(
                           playerInfo.getHammerName() + " Cooldown"
                       )
                       / 2),
                32,
                0xffffffff
    ...
    public static void renderJumps(Minecraft mc) {
        ...
        if (player == null || player.ridingEntity == null
            || !(player.ridingEntity instanceof EntityMoa)) {
            return;
        }
    ...
    public static void
    renderAetherPortal(float timeInPortal, ScaledResolution scaledRes) {
        ...
        tessellator.startDrawingQuads();
        tessellator.addVertexWithUV(
            0.0D, (double) scaledRes.getScaledHeight(), -90.0D, (double) f, (double) f3
        );
        tessellator.addVertexWithUV(
        ...
        tessellator.addVertexWithUV(
            (double) scaledRes.getScaledWidth(), 0.0D, -90.0D, (double) f2, (double) f1
        );
    ...
    public static void renderBossHP(Minecraft mc) {
        ...
        if (player.getFocusedBoss() != null) {
            ...
            int healthRemaining
                = (int) (boss.getBossHealth() / boss.getMaxBossHealth() * 256F);
            int width = scaledresolution.getScaledWidth();
    ...
    public static void
    drawTexturedModalRect(float x, float y, float u, float v, float width, float height) {
        float zLevel = -90.0F;
