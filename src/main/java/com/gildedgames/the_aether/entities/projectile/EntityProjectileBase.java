...
public abstract class EntityProjectileBase
    extends Entity implements IProjectile, IThrowableEntity {
    public int xTile = -1;
    public int yTile = -1;
    public int zTile = -1;
    public Block inTile;
    ...
    public EntityProjectileBase(World world, EntityLivingBase thrower) {
        this(
            world,
            thrower.posX,
            thrower.posY + (double) thrower.getEyeHeight() - 0.10000000149011612D,
            thrower.posZ
        );
    ...
    public void shoot(
        ...
        this.setThrowableHeading(
            (double) f, (double) f1, (double) f2, velocity, inaccuracy
        );
    ...
    public void onUpdate() {
        ...
        if (this.inGround) {
            if (this.worldObj.getBlock(this.xTile, this.yTile, this.zTile)
                == this.inTile) {
                ++this.ticksInGround;
        ...
        MovingObjectPosition movingobjectposition
            = this.worldObj.rayTraceBlocks(vec3, vec31);
        ...
        for (this.rotationPitch
             = (float) (Math.atan2(this.motionY, (double) f1) * 180.0D / Math.PI);
