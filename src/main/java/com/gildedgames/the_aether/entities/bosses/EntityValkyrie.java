...
public class EntityValkyrie extends EntityAetherMob {
    ...
    public boolean interact(EntityPlayer entityplayer) {
        ...
        if (stack != null && stack.getItem() == ItemsAether.victory_medal
            ...
            if (stack.stackSize >= 10) {
                this.chatItUp(
                    entityplayer,
                    StatCollector.translateToLocal("gui.valkyrie.dialog.medal.1")
                );
            } else if (stack.stackSize >= 5) {
                this.chatItUp(
                    entityplayer,
                    StatCollector.translateToLocal("gui.valkyrie.dialog.medal.2")
                );
            } else {
                this.chatItUp(
                    entityplayer,
                    StatCollector.translateToLocal("gui.valkyrie.dialog.medal.3")
                );
            }
        } else {
            ...
            if (line == 2) {
                this.chatItUp(
                    entityplayer, StatCollector.translateToLocal("gui.valkyrie.dialog.1")
                );
            } else if (line == 1) {
                this.chatItUp(
                    entityplayer, StatCollector.translateToLocal("gui.valkyrie.dialog.2")
                );
            } else {
                this.chatItUp(
                    entityplayer, StatCollector.translateToLocal("gui.valkyrie.dialog.3")
                );
            }
        }
    ...
    public void teleport(double x, double y, double z, int rad) {
        ...
        if (!flag) {
            ...
            this.moveForward = this.moveStrafing = this.rotationPitch = this.rotationYaw
                = 0.0F;
    ...
    public void onUpdate() {
        ...
        if (!this.onGround && this.getAttackTarget() != null && this.lastMotionY >= 0.0D
            && this.motionY < 0.0D && getDistanceToEntity(this.getAttackTarget()) <= 16F
        ...
        if (this.getAttackTarget() instanceof EntityPlayer) {
            ...
            if (this.getHealth() <= 0) {
                ...
                if (pokey == 2) {
                    chatItUp(
                        player,
                        StatCollector.translateToLocal("gui.valkyrie.dialog.defeated.1")
                    );
                } else if (pokey == 1) {
                    chatItUp(
                        player,
                        StatCollector.translateToLocal("gui.valkyrie.dialog.defeated.2")
                    );
                } else {
                    chatItUp(
                        player,
                        StatCollector.translateToLocal("gui.valkyrie.dialog.defeated.3")
                    );
                }
                ...
                if (pokey == 2) {
                    ...
                    chatItUp(
                        ...
                        StatCollector.translateToLocalFormatted(
                            "gui.valkyrie.dialog.playerdead.2", player.getDisplayName()
                        )
                    );
                } else {
                    chatItUp(
        ...
        if (ds.getEntity() instanceof EntityPlayer
            ...
            if (this.getAttackTarget() == null) {
                ...
                if (pokey == 2) {
                    chatItUp(
                        player,
                        StatCollector.translateToLocal("gui.valkyrie.dialog.attack.1")
                    );
                } else if (pokey == 1) {
                    chatItUp(
                        player,
                        StatCollector.translateToLocal("gui.valkyrie.dialog.attack.2")
                    );
                } else {
                    chatItUp(
                        player,
                        StatCollector.translateToLocal("gui.valkyrie.dialog.attack.3")
                    );
                }
    ...
    public boolean attackEntityAsMob(Entity entity) {
        ...
        if (this.attackTime <= 0 && entity.boundingBox.maxY > this.boundingBox.minY
            ...
            if (entity != null && this.getAttackTarget() != null
                && entity == getAttackTarget() && entity instanceof EntityPlayer) {
                    ...
                    if (pokey == 2) {
                        chatItUp(
                            ...
                            StatCollector.translateToLocal(
                                "gui.valkyrie.dialog.playerdead.1"
                            )
                        );
                    } else if (pokey == 1) {
                        chatItUp(
                        ...
                        chatItUp(
                            (EntityPlayer) this.getAttackTarget(),
                            StatCollector.translateToLocal(
                                "gui.valkyrie.dialog.playerdead.3"
                            )
                        );
                    }
