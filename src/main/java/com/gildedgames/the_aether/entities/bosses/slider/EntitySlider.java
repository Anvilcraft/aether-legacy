...
public class EntitySlider extends EntityFlying implements IAetherBoss {
    ...
    public void updateEntityActionState() {
        ...
        if (!this.worldObj.isRemote) {
            if (this.getAttackTarget() == null || this.getAttackTarget().isDead
                || this.getAttackTarget().getHealth() <= 0.0F) {
                this.reset();
            ...
            if (this.isMoving()) {
                if (this.isCollided) {
                    ...
                    if (this.direction == EnumFacing.UP) {
                        ...
                        if (this.boundingBox.minY
                            > this.getAttackTarget().boundingBox.minY + 0.35D) {
                            this.stop();
                        ...
                        if (this.boundingBox.minY
                            < this.getAttackTarget().boundingBox.minY - 0.25D) {
                            this.stop();
                            this.moveTime = this.criticalCondition() ? 4 : 8;
                        }
                    } else if (this.direction == EnumFacing.EAST) {
                ...
                if (this.moveTime > 0) {
                    ...
                    b = Math.abs(
                        this.boundingBox.minY - this.getAttackTarget().boundingBox.minY
                    );
    ...
    private void closeDoor() {
        if (checkIsAir(
            ...
            //EAST
            this.doorStart
                = new int[] { this.dungeonX + 15, this.dungeonY + 1, this.dungeonZ + 6 };
            this.doorEnd
                = new int[] { this.dungeonX + 15, this.dungeonY + 4, this.dungeonZ + 9 };
            ...
            //WEST
            this.doorStart
                = new int[] { this.dungeonX, this.dungeonY + 1, this.dungeonZ + 6 };
            this.doorEnd
                = new int[] { this.dungeonX, this.dungeonY + 4, this.dungeonZ + 9 };
            ...
            //SOUTH
            this.doorStart
                = new int[] { this.dungeonX + 6, this.dungeonY + 1, this.dungeonZ + 15 };
            this.doorEnd
                = new int[] { this.dungeonX + 9, this.dungeonY + 4, this.dungeonZ + 15 };
            ...
            //NORTH
            this.doorStart
                = new int[] { this.dungeonX + 6, this.dungeonY + 1, this.dungeonZ };
            this.doorEnd
                = new int[] { this.dungeonX + 9, this.dungeonY + 4, this.dungeonZ };
    ...
    public void applyEntityCollision(Entity entity) {
        if (this.isAwake() && this.isMoving()) {
            boolean flag
                = entity.attackEntityFrom(new EntityDamageSource("crush", this), 6);

            if (flag && entity instanceof EntityLivingBase) {
                EntityLivingBase collidedEntity = (EntityLivingBase) entity;
                collidedEntity.addVelocity(
                    collidedEntity.motionY, 0.35D, collidedEntity.motionZ
                );
                this.worldObj.playSoundEffect(
                    this.posX,
                    this.posY,
    ...
    protected void dropFewItems(boolean wasRecentlyHit, int lootingModifier) {
        this.dropItem(
            Item.getItemFromBlock(BlocksAether.carved_stone), 7 + rand.nextInt(3)
        );
    ...
    public boolean attackEntityFrom(DamageSource ds, float var2) {
        if (ds.getEntity() == null || !(ds.getEntity() instanceof EntityPlayer)
            || ds.isProjectile() || ds.isMagicDamage() || ds.isExplosion()
            || ds.isFireDamage()) {
            return false;
        }
        ...
        if (!((EntityPlayer) ds.getEntity()).capabilities.isCreativeMode
            && ds.getSourceOfDamage().getDistance(this.posX, this.posY, this.posZ) > 6
            && !this.isAwake()) {
            ...
            boolean isTCPickaxe = stack.getItem().getClass().getName().equals(
                "tconstruct.items.tools.Pickaxe"
            );

            if (!stack.getItem().canHarvestBlock(Blocks.stone, stack) && !isTCPickaxe) {
                this.sendMessage(
                    player, StatCollector.translateToLocal("gui.slider.notpickaxe")
                );
        ...
        if (flag) {
            for (int size = 0; size < (this.getHealth() <= 0 ? 2 : 4); size++) {
                double a = this.posX
                    + ((this.rand.nextFloat() - this.rand.nextFloat()) * 1.5D);
            ...
            if (this.getHealth() <= 0 || this.isDead) {
                ...
                for (int i = 0; i < dungeonPlayers.size(); ++i) {
                    ...
                    if (entity instanceof EntityPlayer) {
                        ((EntityPlayer) entity)
                            .triggerAchievement(AchievementsAether.defeat_bronze);
                    }
                }
    ...
    public void reset() {
        ...
        this.setPositionAndUpdate(
            this.dungeonX + 8, this.dungeonY + 2, this.dungeonZ + 8
        );
    }
