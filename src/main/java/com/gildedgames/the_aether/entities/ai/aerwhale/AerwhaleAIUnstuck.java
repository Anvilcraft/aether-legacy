...
public class AerwhaleAIUnstuck extends EntityAIBase {
    ...
    public void updateTask() {
        double[] posUp = new double[] { this.aerwhale.posX,
                                        this.aerwhale.posY + 3,
                                        this.aerwhale.posZ };
        double[] posDown = new double[] { this.aerwhale.posX,
                                          this.aerwhale.posY - 1,
                                          this.aerwhale.posZ };
        double[] posEast = new double[] { this.aerwhale.posX + 1,
                                          this.aerwhale.posY + 2,
                                          this.aerwhale.posZ };
        double[] posWest = new double[] { this.aerwhale.posX - 3,
                                          this.aerwhale.posY + 2,
                                          this.aerwhale.posZ };
        double[] posSouth = new double[] { this.aerwhale.posX,
                                           this.aerwhale.posY + 2,
                                           this.aerwhale.posZ + 1 };
        double[] posNorth = new double[] { this.aerwhale.posX,
                                           this.aerwhale.posY + 2,
                                           this.aerwhale.posZ - 3 };

        if (this.checkRegion(
                new double[] { posUp[0] - 2, posUp[1], posUp[2] - 2 }, posUp
            )) {
            this.aerwhale.motionY -= 0.002;
        }
        if (this.checkRegion(
                new double[] { posDown[0] - 2, posDown[1], posDown[2] - 2 }, posDown
            )) {
            this.aerwhale.motionY += 0.002;
        }
        if (this.checkRegion(
                new double[] { posEast[0], posEast[1] - 2, posEast[2] - 2 }, posEast
            )) {
            this.aerwhale.motionX -= 0.002;
            this.aerwhale.motionY += 0.002;
        }
        if (this.checkRegion(
                new double[] { posWest[0], posWest[1] - 2, posWest[2] - 2 }, posWest
            )) {
            this.aerwhale.motionX += 0.002;
            this.aerwhale.motionY += 0.002;
        }
        if (this.checkRegion(
                new double[] { posSouth[0] - 2, posSouth[1] - 2, posSouth[2] }, posSouth
            )) {
            this.aerwhale.motionZ -= 0.002;
            this.aerwhale.motionY += 0.002;
        }
        if (this.checkRegion(
                new double[] { posNorth[0] - 2, posNorth[1] - 2, posNorth[2] }, posNorth
            )) {
            this.aerwhale.motionZ += 0.002;
            this.aerwhale.motionY += 0.002;
        }
