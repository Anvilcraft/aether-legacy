...
public class ZephyrAIShootTarget extends EntityAIBase {
    ...
    public ZephyrAIShootTarget(EntityZephyr zephyr) {
        ...
        this.base
            = (this.zephyr.getRNG().nextFloat() - this.zephyr.getRNG().nextFloat()) * 0.2F
            + 1.0F;
        this.setMutexBits(4);
    }
    ...
    public void updateTask() {
        ...
        if (this.zephyr.getAttackTarget() == null) {
            ...
            this.zephyr.setAttackTarget(
                this.worldObj.getClosestVulnerablePlayerToEntity(this.zephyr, 100D)
            );
        } else {
            if (this.zephyr.getAttackTarget() instanceof EntityPlayer
                && (((EntityPlayer) this.zephyr.getAttackTarget())
                        .capabilities.isCreativeMode)) {
                this.zephyr.setAttackTarget(null);
                return;
            }

            if (this.zephyr.getAttackTarget().getDistanceSqToEntity(this.zephyr) < 4096.0D
                ...
                double y = (this.zephyr.getAttackTarget().boundingBox.minY
                            + (this.zephyr.getAttackTarget().height / 2.0F))
                    - (this.zephyr.posY + (this.zephyr.height / 2.0F));
                ...
                if (this.attackCounter == 10) {
                    this.zephyr.playSound(
                        "aether_legacy:aemob.zephyr.call", 3F, this.base
                    );
                    ...
                    EntityZephyrSnowball projectile
                        = new EntityZephyrSnowball(this.worldObj, this.zephyr, x, y, z);
                    Vec3 lookVector = this.zephyr.getLook(1.0F);

                    projectile.posX = this.zephyr.posX + lookVector.xCoord * 4D;
                    projectile.posY
                        = this.zephyr.posY + (double) (this.zephyr.height / 2.0F) + 0.5D;
