...
public abstract class EntityMountable extends EntityAetherAnimal {
    ...
    public void updateRider() {
        if (this.canDismount() && this.riddenByEntity instanceof EntityPlayer) {
            PlayerAether playerAether
                = PlayerAether.get((EntityPlayer) this.riddenByEntity);
    ...
    public void moveEntityWithHeading(float par1, float par2) {
        ...
        if (entity instanceof EntityPlayer) {
            ...
            if (this.jumpPower > 0.0F && !this.isMountJumping()
                && (this.onGround || this.canJumpMidAir)) {
    ...
    public float getAIMoveSpeed() {
        return this.riddenByEntity != null ? this.getMountedMoveSpeed()
