...
public class EntitySheepuff extends EntityAetherAnimal {
    private SheepuffAIEatAetherGrass entityAIEatGrass
        = new SheepuffAIEatAetherGrass(this);
    ...
    public boolean getCanSpawnHere() {
        return this.rand.nextInt(AetherConfig.getSheepuffSpawnrate()) == 0
    ...
    protected void dropFewItems(boolean var1, int ammount) {
        if (!this.getSheared()) {
            this.entityDropItem(
                new ItemStack(
                    Blocks.wool, 1 + this.rand.nextInt(2), this.getFleeceColor()
                ),
                0.0F
            );
        }
    ...
    public float getHeadRotationAngleX(float p_70890_1_) {
        if (this.sheepTimer > 4 && this.sheepTimer <= 36) {
            ...
            return ((float) Math.PI / 5F)
                + ((float) Math.PI * 7F / 100F) * MathHelper.sin(f * 28.7F);
        } else {
            return this.sheepTimer > 0 ? ((float) Math.PI / 5F)
                                       : this.rotationPitch * 0.017453292F;
        }
    }
            ...
            if (!this.worldObj.isRemote) {
                ...
                for (int j = 0; j < i; j++) {
                    EntityItem entityitem = this.entityDropItem(
                        new ItemStack(Blocks.wool, 1, this.getFleeceColor()), 1.0F
                    );
