...
public class EntitySwet extends EntityMountable {
    ...
    public void entityInit() {
        ...
        this.dataWatcher.addObject(
            21, (byte) this.rand.nextInt(EnumSwetType.values().length)
        );
    }
    ...
    public void onEntityUpdate() {
        ...
        if (this.getAttackTarget() instanceof EntityPlayer) {
            if (this.isPlayerFriendly((EntityPlayer) this.getAttackTarget())
                || this.isFriendly()) {
    ...
    public void onUpdate() {
        ...
        if (!this.hasPrey()) {
            for (int i = 0; i < 5; i++) {
                double d = (float) this.posX
                    + (this.rand.nextFloat() - this.rand.nextFloat()) * 0.3F;
                double d1 = (float) this.posY + this.height;
                double d2 = (float) this.posZ
                    + (this.rand.nextFloat() - this.rand.nextFloat()) * 0.3F;
                this.worldObj.spawnParticle(
                    "splash", d, d1 - 0.25D, d2, 0.0D, 0.0D, 0.0D
                );
            }
        }
        ...
        if (this.worldObj.isRemote) {
            ...
            if (this.onGround) {
                this.swetHeight
                    = this.swetHeight < 1.0F ? this.swetHeight += 0.25F : 1.0F;
    ...
    protected void updateEntityActionState() {
        ...
        EntityPlayer entityplayer
            = this.worldObj.getClosestVulnerablePlayerToEntity(this, 25.0D);

        if (entityplayer != null) {
            if (entityplayer.isEntityAlive()
                && !entityplayer.capabilities.disableDamage) {
                if (!this.isPlayerFriendly(entityplayer) && !this.isFriendly()
                    && !this.hasPrey()) {
    ...
    public boolean isPlayerFriendly(EntityPlayer player) {
        ...
        return iPlayerAether.getAccessoryInventory().wearingAccessory(
            new ItemStack(ItemsAether.swet_cape)
        );
    }
    ...
    public void dissolveSwet() {
        ...
        if (this.getDeathSound() != null)
            this.playSound(
                this.getDeathSound(), this.getSoundVolume(), this.getSoundPitch()
            );
    ...
    public boolean getCanSpawnHere() {
        return this.rand.nextInt(AetherConfig.getSwetSpawnrate()) == 0
