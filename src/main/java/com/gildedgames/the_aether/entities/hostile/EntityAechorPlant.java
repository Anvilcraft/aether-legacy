...
public class EntityAechorPlant extends EntityAetherAnimal {
    ...
    public EntityAechorPlant(World world) {
        ...
        this.setSize(
            0.75F + ((float) this.getSize() * 0.125F),
            0.5F + ((float) this.getSize() * 0.075F)
        );
    }
    ...
    public boolean getCanSpawnHere() {
        return this.rand.nextInt(AetherConfig.getAechorPlantSpawnrate()) == 0
    ...
    public void onLivingUpdate() {
        ...
        if (this.getEntityToAttack() == null) {
            EntityPlayer player
                = this.worldObj.getClosestVulnerablePlayerToEntity(this, 10.0F);
    ...
    public void shootAtPlayer() {
        ...
        EntityPoisonNeedle poisonNeedle
            = new EntityPoisonNeedle(this.worldObj, this, 0.5F);

        poisonNeedle.posY = this.posY + 1D;

        this.playSound(
            "random.bow", 1.0F, 1.2F / (this.getRNG().nextFloat() * 0.2F + 0.9F)
        );
                ...
                if (--heldItem.stackSize == 0) {
                    player.setCurrentItemOrArmor(
                        ...
                        new ItemStack(
                            ...
                            1,
                            EnumSkyrootBucketType.Poison.meta
                        )
                    );
