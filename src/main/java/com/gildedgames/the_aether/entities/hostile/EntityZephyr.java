...
public class EntityZephyr extends EntityFlying implements IMob {
    ...
    protected void updateEntityActionState() {
        ...
        if (d3 < 1.0D || d3 > 3600.0D) {
            this.waypointX
                = this.posX + (double) ((this.rand.nextFloat() * 2.0F - 1.0F) * 16.0F);
            this.waypointY
                = this.posY + (double) ((this.rand.nextFloat() * 2.0F - 1.0F) * 16.0F);
            this.waypointZ
                = this.posZ + (double) ((this.rand.nextFloat() * 2.0F - 1.0F) * 16.0F);
        }
        ...
        if (this.courseChangeCooldown-- <= 0) {
            ...
            if (this.isCourseTraversable(
                    this.waypointX, this.waypointY, this.waypointZ, d3
                )) {
        ...
        if (this.getAttackTarget() == null) {
            ...
            this.setAttackTarget(
                this.worldObj.getClosestVulnerablePlayerToEntity(this, 100D)
            );
        } else {
            ...
            if (this.getAttackTarget().getDistanceSqToEntity(this) < 4096.0D
                && this.canEntityBeSeen(this.getAttackTarget())) {
                double x = this.getAttackTarget().posX - this.posX;
                double y = (this.getAttackTarget().boundingBox.minY
                            + (this.getAttackTarget().height / 2.0F))
                ...
                if (this.attackCounter == 10) {
                    ...
                    EntityZephyrSnowball projectile
                        = new EntityZephyrSnowball(this.worldObj, this, x, y, z);
    ...
    private boolean isCourseTraversable(
        double p_70790_1_, double p_70790_3_, double p_70790_5_, double p_70790_7_
    ) {
